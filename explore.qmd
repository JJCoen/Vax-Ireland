...
irl_post_vax[, vax_total := vax_rates_vec][, vax_total := 
                                               round(vax_total, 1)]
irl_post_vax |> 
    head(10) |> 
    gt()
...
plot_ly(x = ~year, y = ~xs_avg_pc, 
    ...
    add_trace(x = ~year, y = ~vax_total, 
              ...
              #showlegend = TRUE,
              hoverinfo = "text",
              text = ~paste("Vax Rates ", 
                            "<br> Year: ", year, "% population, first dose: ", vax_total) ) |>
...
The age group 5-14 is not given for Method 2C since the Linear Model does not fit to the observed data.
...
# Extract vaccination percent for 0-4 age group
vax_0 <- vax_euro[age_group == "0-4", ]
...
vax_0[, week_day := to_week(week)] #[, week := NULL]
```

Fit a Logistic Function to the percentage vacccinated.
...
p_0_4 <- ggplot(vax_0, aes(x = week_day,
                       y=vax_pc,
                       text = paste('percent: ', vax_pc,
                           '<br>Date: ', as.Date(week_day)),
                       group = age_group) )
p <- p_0_4 + geom_point(size = 0.2) + 
    stat_smooth(method="glm", se=FALSE, colour = "darkblue",
              method.args = list(family=quasibinomial) ) +
    labs(#title = 'Cumulative First Dose', 
         x = "", y = 'Age Group percent') +
